...
const PROVIDERS: ProviderConfig[] = [
 ...
 { id: 'custom', name: 'Custom', icon: '', color: 'from-gray-500/20 to-slate-500/20', models: [], defaultModel: 'custom-model', apiKey: '', status: 'idle', customEndpoint: '' },
]
...
Output: # Title>description\n## Mandatory Rules\n## Workflows\n## Quick Reference\n## Red Flags\nKeep safety/error/numbered procedures verbatim. Drop motivation, merge variants, convert prose to bullets. Markdown only.`,
 ...
 <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-600 via-amber-500 to-yellow-700 flex items-center justify-center text-sm shadow-lg shadow-amber-600/20"></div>
 <div>
 ...
 <button onClick={() => setShowSettings(true)} className={`text-xs px-4 py-2 rounded-full font-medium transition-all ${prov?.apiKey ? 'bg-white/[0.06] hover:bg-white/[0.1] text-gray-400 border border-[#e0d8c8]' : 'bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow-lg shadow-amber-600/20 hover:shadow-amber-600/25'}`}>
 {prov?.apiKey ? ' 模型设置' : ' 开始配置'}
 </button>
 </div>
 </div>
 </nav>
 ...
 <div className="absolute inset-0 bg-gradient-to-b from-amber-600/[0.05] via-amber-500/[0.03] to-transparent pointer-events-none" />
 ...
 <button onClick={() => setMode('fusion')} className={`px-4 py-2 rounded-lg text-xs font-medium transition-all ${mode === 'fusion' ? 'bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow-lg shadow-amber-600/20' : 'text-gray-500 hover:text-gray-700'}`}> 合并压缩</button>
 ...
 : 'bg-gradient-to-r from-amber-600 via-amber-500 to-yellow-700 hover:from-blue-500 hover:via-violet-500 hover:to-purple-500 text-white shadow-lg shadow-amber-600/20'
 }`}>
 {loading ? (
 ...
 <div className="absolute -top-2 -left-2 w-6 h-6 rounded-full bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center text-[10px] font-bold shadow-lg shadow-amber-600/20">{s.step}</div>
 ...
 {p.models.map(m => <button key={m} onClick={() => { setModel(m); setProvId(p.id) }} className={`text-[10px] px-2.5 py-1 rounded-lg transition ${provId === p.id && model === m ? 'bg-violet-600 text-white shadow shadow-amber-600/20' : 'bg-white/90 text-gray-500 hover:bg-white/[0.08]'}`}>{m}</button>)}
 </div>
 )}
 {p.apiKey && (
 <button onClick={() => selectProv(p.id)} className={`w-full py-2 rounded-lg text-xs font-medium transition-all ${provId === p.id ? 'bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow shadow-amber-600/20' : 'bg-white/[0.06] text-gray-400 hover:bg-white/[0.1]'}`}>
